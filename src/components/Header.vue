  ...
  <div class="header" :class="{ 'content-collapse': collapseLocal }">
    <div class="header_arera">
      <div class="header-right">
        <div style="z-index: 9; margin: 0.2rem">
          <label class="dropdown">
            <div class="dd-button">{{ languageMcs === "cn" ? "中" : "EN" }}</div>
            <input type="checkbox" class="dd-input" id="test" />
            <ul class="dd-menu">
              <li @click="handleSetLanguage('en')">EN</li>
              <li @click="handleSetLanguage('cn')">中</li>
            </ul>
          </label>
          <!-- <select class="lang_style" @change="handleSetLanguage($event)">
                        <option value="" selected hidden>{{languageMcs === 'cn'? '中':'EN'}}</option>
        ...
        <div :class="{ online: addrChild, 'feh-metamask': 1 == 1 }">
          <div v-if="!addrChild" class="logged_in filter_status">
            <el-tooltip
              class="item"
              effect="dark"
              :content="$t('fs3Login.toptip_03')"
              placement="bottom"
            >
              <img src="@/assets/images/metamask.png" @click="metamaskLogin" />
            </el-tooltip>
            <span class="text" @click="metamaskLogin">{{
              $t("fs3.Connect_Wallet")
            }}</span>
          </div>
          ...
          <div v-else class="logged_in">
            ...
            <el-button class="text textTrue pcShow" @click="signOutFun">{{
              $t("fs3.Disconnect")
            }}</el-button>
          </div>
        </div>
    ...
    >
      ...
      <div class="share">
        <el-button @click="shareTo">
          ...
          >
            <path
              d="M852.77 889.05H171.23A36.27 36.27 0 0 1 135 852.78V171.22A36.27 36.27 0 0 1 171.23 135H375a36.28 36.28 0 0 1 0 72.55H207.5v609h609V649a36.28 36.28 0 1 1 72.55 0v203.78a36.27 36.27 0 0 1-36.28 36.27z"
              fill="#0b318f"
              p-id="3724"
            ></path>
            <path
              d="M407.15 653.13a36.28 36.28 0 0 1-25.66-61.93L747.66 225A36.28 36.28 0 1 1 799 276.34L432.8 642.51a36.17 36.17 0 0 1-25.65 10.62z"
              fill="#0b318f"
              p-id="3725"
            ></path>
            <path
              d="M852.78 414.94V171.23H609.06l140.42 103.29 103.3 140.42z"
              fill="#0b318f"
              p-id="3726"
            ></path>
            <path
              d="M852.78 451.22a36.29 36.29 0 0 1-29.23-14.78l-100-136-136-100a36.28 36.28 0 0 1 21.5-65.5h243.72a36.28 36.28 0 0 1 36.28 36.28V415a36.28 36.28 0 0 1-36.27 36.27zM719.6 207.5l51.4 37.8a36.23 36.23 0 0 1 7.7 7.7l37.8 51.38V207.5z"
              fill="#0b318f"
              p-id="3727"
            ></path>
          </svg>
        ...
        <el-button v-else>
          ...
          >
            <path
              d="M512 998.4c-268.288 0-486.4-218.112-486.4-486.4S243.712 25.6 512 25.6s486.4 218.112 486.4 486.4-218.112 486.4-486.4 486.4z m0-896c-225.792 0-409.6 183.808-409.6 409.6s183.808 409.6 409.6 409.6 409.6-183.808 409.6-409.6-183.808-409.6-409.6-409.6z"
              p-id="5620"
              fill="#0b318f"
            ></path>
            <path
              d="M457.728 680.448c-9.728 0-19.456-3.584-27.136-11.264l-150.528-150.016c-14.848-14.848-14.848-39.424 0-54.272 14.848-14.848 39.424-14.848 54.272 0l123.392 122.88 231.936-230.912c14.848-14.848 39.424-14.848 54.272 0 14.848 14.848 14.848 39.424 0 54.272l-259.072 258.048c-7.68 7.168-17.408 11.264-27.136 11.264z"
              p-id="5621"
              fill="#0b318f"
            ></path>
          </svg>
...
export default {
  ...
  computed: {
    ...
    metaNetworkInfo() {
      return this.$store.getters.metaNetworkInfo
        ? JSON.parse(JSON.stringify(this.$store.getters.metaNetworkInfo))
        : {};
    },
    reverse() {
  ...
  watch: {
    $route: function (to, from) {
      if (
        (to.name === "register" && from.name === "login") ||
        (to.name === "login" && from.name === "register")
      ) {
            ...
            case 80001:
              ...
              if (_this.meta) {
                if (
                  ...
                  _this.$route.query.redirect != "/supplierAllBack"
                ) {
                  // 防止登录后需要跳转到指定页面
      ...
      axios
        ...
        .then((json) => {
          if (json.data.status == "success") {
            ...
            _this.$root.SWAN_PAYMENT_CONTRACT_ADDRESS =
              json.data.data.payment_contract_address;
      ...
      try {
        if (netId != 80001) {
          ...
          if (_this.$root.SWAN_PAYMENT_CONTRACT_ADDRESS) {
            ...
            // 查询剩余代币余额为：
            contract_erc20.methods
              .balanceOf(_this.metaAddress)
              .call()
              .then((balance) => {
