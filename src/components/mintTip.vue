...
<script>
    ...
    import nftContractAbiNew from '../abi/NFT.json'
    export default {
        name: "mint_tip",
        data() {
            return {
        ...
        methods: {
                ...
                that.$refs[formName].validate(async (valid) => {
                    if (valid) {
                        if(that.metaAddress){
                            ...
                            let nftContract = new web3.eth.Contract(
                                nftContractAbiNew,
                                "0x59cfeA631C38ab096C93f9B2Acd361351c00cF89",
                                { from: that.metaAddress, gas: web3.utils.toHex( that.$root.PAY_GAS_LIMIT) },
                            )
                            
                            console.log("dau ma tao o day ")
                            const mintResult = await that.mintContractNew(nftContract, nftUrl)
            ...
            async mintContractNew(nftContract, nftUrl) {
                try {
                    console.log('start mint contract')
                    const transaction = await nftContract.methods
                    .mintToken(nftUrl)
                    ...
                    console.log("trasact",transaction)
                    that.tokenId  = transaction.events.TokenMinted.returnValues.tokenId
