    ...
    <div class="form">
      ...
      <div class="form_table">
        ...
        >
          ...
          <el-table-column prop="Switch" min-width="100" :label="$t('uploadFile.uploadIchill')">
            <template slot-scope="scope">
              <div class="hot-cold-box">
                <el-button class="uploadBtn blue" type="primary"
                    v-if="tableData[scope.$index].file_name.split('.')[1]=='mp4' || tableData[scope.$index].file_name.split('.')[1]=='mov'"
                    @click.stop="uploadIchill(tableData[scope.$index])">{{$t('uploadFile.import')}}</el-button>
                <el-button class="uploadBtn grey opacity" type="primary"
                    v-else
                    :disabled="true">
                   {{$t('uploadFile.import')}}
                </el-button>
          ...
          <!-- <el-table-column prop="Switch" min-width="100" :label="$t('uploadFile.delete')">
            <template slot-scope="scope">
              <div class="hot-cold-box">
                <el-button class="uploadBtn blue" type="primary"
                    @click.stop="uploadIchill(tableData[scope.$index])">{{$t('uploadFile.delete')}}</el-button>
              </div>
            </template>
          </el-table-column> -->
        </el-table>
 ...
 <!-- @getPay="getPay" -->
    ...
    <pay-tip v-if="payVisible" :payVisible="payVisible" 
        ...
        <el-dialog title="" :visible.sync="mintTransaction" :width="width"
            ...
            <a :href="'https://testnets.opensea.io/assets/mumbai/'+mint_address+'/'+tokenId" target="_blank">
            <!-- :href="'https://testnets.opensea.io/assets/mumbai/'+mint_address+'/'+tokenId" -->
            {{$t('uploadFile.View_Your_NFT_OpenSea')}}
            </a>
...
export default {
  ...
  methods: {
    ...
    uploadIchill(row){
      window.open(this.$urlImport+'import-video?ipfsurl='+row.ipfs_url+'&title='+encodeURIComponent(row.file_name)); 
    },
    refundClick(row){
        let _this = this
